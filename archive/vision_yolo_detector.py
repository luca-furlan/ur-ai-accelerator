...
Integrazione completa: Camera Orbecc → YOLOv8 → Publish detections
"""
...
class VisionYOLODetector(Node):
    """
    Nodo ROS2 che:
    1. Sottoscrive ai topic della camera Orbecc (RGB + Depth)
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"Inizializzazione Vision YOLO Detector...")
        ...
        if YOLO_AVAILABLE:
            ...
            self.get_logger().error("❌ YOLOv8 non disponibile")
    ...
    def depth_callback(self, msg: Image):
        ...
        try:
            # Depth tipicamente in mm o m (uint16 o float32)
        ...
        try:
            ...
            if len(results) > 0:
                ...
                for box in boxes:
                    ...
                    # Detection 2D (ROS standard)
                    det_2d = Detection2D()
                    det_2d.bbox.center.position.x = float(center_x)
                    det_2d.bbox.center.position.y = float(center_y)
                    det_2d.bbox.size_x = float(xyxy[2] - xyxy[0])
                    det_2d.bbox.size_y = float(xyxy[3] - xyxy[1])
            ...
            if self.frame_count % 100 == 0:
                self.get_logger().info(
                    ...
                    f"Current: {len(detections_2d)} objects"
                )
    ...
    def get_3d_point(self, px: int, py: int) -> Optional[Tuple[float, float, float]]:
        ...
        try:
            ...
            # Ottieni depth (in mm tipicamente per Orbecc)
