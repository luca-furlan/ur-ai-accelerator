...
try:
    ...
    if "PLAYING" not in program_state:
        print(f"❌ Programma NON in PLAYING: {program_state}")
        print("💡 Configura il robot con External Control URCap e avvia programma in PLAYING")
        exit(1)
    else:
...
echo
echo "Se vedi errori 'Pipeline producer overflowed':"
echo "  → Ferma altri processi che usano RTDE (web interface, altri script)"
echo
